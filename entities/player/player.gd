...
#const Teleporter = preload("res://entities/teleporter/teleporter.tscn")
...
@onready var utilities = $Utilities
...
func _ready():
	current_utility = utilities.get_children()[0]
...
func drop_utility():
	if map_utilities.get_children().size() < stats.max_utility_drops:
		var util = current_utility.duplicate()
		util.show()
		util.init(self)
...
func toggle_utility():
	var index = utilities.get_children().find(current_utility)
	if index == utilities.get_children().size() - 1:
		current_utility = utilities.get_children()[0]
	else:
		current_utility = utilities.get_children()[index + 1]
