...
const UtilitySelector = preload("res://targeting/utility_selector/utility_selector.tscn")
...
func setup(simulation_player):
	...
	var utils = player.map_utilities.get_children().duplicate()
	for util in utils:
		if util.queued:
			utils.erase(util)
