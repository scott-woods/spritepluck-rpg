...
func start(area_data : Resource, target_spawn : String):
	#set area data
	current_area_data = area_data
	
	#generate dungeon and get dungeon map
	dungeon_map = dungenerator.generate_dungeon(area_data, target_spawn)
	
	#move to room with coordinates (0, 0) in map
	move_to_room(Vector2.ZERO, target_spawn)
...
func _on_door_entered(target_coordinates : Vector2, target_spawn : String):
	rooms_cleared += 1
	move_to_room(target_coordinates, target_spawn)
